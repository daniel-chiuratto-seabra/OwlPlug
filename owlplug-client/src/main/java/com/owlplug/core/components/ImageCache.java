...
public class ImageCache {
    ...
    /**
     * Asynchronously retrieve or persist an image in cache from url.
     *
     * @param url Image url
    ...
    public Image get(String url, String type, boolean asyncFetch) {
        ...
        if (cachedElement != null) {
            try {
                LOGGER.trace("Retrieving image {} from cache", url);
                ...
                LOGGER.error("Error retrieving image from cache", e);
            }
        }
    ...
    /**
     * Asynchronously load an Image from the cache on the given ImageView. If an image
     * doesn't exist in the cache, it will be created retrieving the image from the url.
     *
     * @param url       Image url
    ...
    private void persistImageIntoCache(Cache<String, byte[]> cache, String key, Image image, String type) {
        try {
            LOGGER.trace("Persisting image {} into cache", key);
