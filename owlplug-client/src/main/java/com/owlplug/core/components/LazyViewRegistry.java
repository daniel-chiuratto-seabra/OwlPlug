...
public class LazyViewRegistry {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LazyViewRegistry.class);
    ...
    private final Map<String, Parent> viewRegistry;

    public LazyViewRegistry(final ApplicationContext applicationContext) {
    ...
    private Parent loadFxml(String resource) {
        final var fxMLLoader = new FXMLLoader(getClass().getResource(resource));
        fxMLLoader.setControllerFactory(applicationContext::getBean);
        try {
            return fxMLLoader.load();
        } catch (IOException e) {
            LOGGER.error("View can't be loaded in registry", e);
        }
        return null;
    }
