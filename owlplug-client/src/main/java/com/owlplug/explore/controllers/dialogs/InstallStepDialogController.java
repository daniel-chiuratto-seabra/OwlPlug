...
public class InstallStepDialogController extends AbstractDialogController {
    ...
    @FXML private Accordion stepAccordion;
    @FXML private TitledPane verifyStep;
    @FXML private TitledPane selectDirectoryStep;
    @FXML private TitledPane prepareStep;
    @FXML private ProgressBar progressBar;
    @FXML private Text formatText;
    @FXML private TextField installationDirectoryTextField;
    @FXML private ToggleButton vst2ToggleButton;
    @FXML private ToggleButton vst3ToggleButton;
    @FXML private ToggleButton auToggleButton;
    @FXML private ToggleButton lv2ToggleButton;
    @FXML private Text installationDirectoryText;
    @FXML private Text directoryValidText;
    @FXML private Text directoryOverrideText;
    @FXML private Button directoryChooserButton;
    @FXML private CheckBox directoryOverrideCheckBox;
    ...
    public InstallStepDialogController(final ApplicationDefaults applicationDefaults, final ApplicationPreferences applicationPreferences,
                                       final TelemetryService telemetryService, final DialogManager dialogManager,
                                       final ExploreService exploreService, final LazyViewRegistry lazyViewRegistry,
                                       final ExploreTaskFactory exploreTaskFactory, final ImageCache imageCache) {
        super(applicationDefaults, applicationPreferences, telemetryService, dialogManager, 500, 600);
    ...
    public void initialize() {
        ...
        toggleGroup.selectedToggleProperty().addListener((obs, o, n) -> {
            ...
            if (vst2ToggleButton.equals(obs.getValue())) {
                baseDir = this.getApplicationPreferences().get(ApplicationDefaults.VST_DIRECTORY_KEY, "");
            } else if (vst3ToggleButton.equals(obs.getValue())) {
                baseDir = this.getApplicationPreferences().get(ApplicationDefaults.VST3_DIRECTORY_KEY, "");
            } else if (auToggleButton.equals(obs.getValue())) {
                baseDir = this.getApplicationPreferences().get(ApplicationDefaults.AU_DIRECTORY_KEY, "");
            } else if (lv2ToggleButton.equals(obs.getValue())) {
                baseDir = this.getApplicationPreferences().get(ApplicationDefaults.LV2_DIRECTORY_KEY, "");
            }
            if (baseDir != null) {
        ...
        directoryChooserButton.setOnAction(e -> {
            if (installParams != null) {
                ...
                if (selectedDirectory != null) {
                    ...
                    // reset the confirmation flag if the directory changed
        ...
        continueButton.setOnAction(e -> resumeInstall());
    }
    ...
    private File generateInstallationDirectoryFromBasePath(String baseDirectoryPath) {
        ...
        if (baseDirectoryPath != null && !baseDirectoryPath.isBlank()) {
            ...
            boolean shouldStoreInDirectory = this.getApplicationPreferences().getBoolean(
            ...
            if (shouldStoreInDirectory) {
                String relativeDirectoryPath = this.getApplicationPreferences().get(ApplicationDefaults.STORE_DIRECTORY_KEY, "");
                selectedDirectory = new File(selectedDirectory, relativeDirectoryPath);
            }

            boolean shouldGroupByCreator = this.getApplicationPreferences().getBoolean(ApplicationDefaults.STORE_BY_CREATOR_ENABLED_KEY, false);
            ...
            boolean shouldWrapSubDirectory = this.getApplicationPreferences().getBoolean(ApplicationDefaults.STORE_SUBDIRECTORY_ENABLED, true);
    ...
    public void reset() {
        ...
        vst2ToggleButton.setDisable(!this.getApplicationPreferences().getBoolean(
                ApplicationDefaults.VST2_DISCOVERY_ENABLED_KEY, false));
        vst3ToggleButton.setDisable(!this.getApplicationPreferences().getBoolean(
                ApplicationDefaults.VST3_DISCOVERY_ENABLED_KEY, false));
        auToggleButton.setDisable(!this.getApplicationPreferences().getBoolean(
                ApplicationDefaults.AU_DISCOVERY_ENABLED_KEY, false));
        lv2ToggleButton.setDisable(!this.getApplicationPreferences().getBoolean(
