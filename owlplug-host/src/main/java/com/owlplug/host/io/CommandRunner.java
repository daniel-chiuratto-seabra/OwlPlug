...
public class CommandRunner {
    ...
    public CommandResult run(String... command) throws IOException {
        ...
        try (final var executorService = Executors.newFixedThreadPool(1)) {
            final var stdoutReader = new StreamReader(process.getInputStream());
            final var stdoutFuture = executorService.submit(stdoutReader);
            ...
            try {
                ...
                LOGGER.error("Interrupted while waiting for process");
                process.destroy();
                throw new IOException("Interrupted while waiting for process", e);
            }

            if (!finished) {
                LOGGER.error("Forcibly destroying process after timeout {}ms exceeded.", timeout);
                process.destroyForcibly();
                throw new IOException("Process timeout exceeded: " + timeout + "ms");
            }

            try {
                // Let 1 second for gracefully read and complete the process
