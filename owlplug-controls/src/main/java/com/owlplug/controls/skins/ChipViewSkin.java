...
public class ChipViewSkin<T> implements Skin<ChipView> {
    ...
    private final CustomFlowPane root;
    private final ChipView<T> control;
    ...
    public ChipViewSkin(ChipView<T> control) {
        ...
        root.vgapProperty().addListener((_ -> autoCompletePopup.setShift(root.getVgap() * 2)));
        ...
        final ListChangeListener<T> chipsChangeListeners = change -> {
            while (change.next()) {
