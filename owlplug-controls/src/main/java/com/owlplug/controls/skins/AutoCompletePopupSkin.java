...
/**
 * AutoCompletePopupSkin is an animated popup list view that allows filtering.
...
public class AutoCompletePopupSkin<T> implements Skin<AutoCompletePopup<T>> {
    ...
    public AutoCompletePopupSkin(AutoCompletePopup<T> control) {
        ...
        suggestionList = new ListView<>(control.getFilteredSuggestions()) {
            @Override
        ...
        control.fixedCellSizeProperty().addListener(_ -> suggestionList.setFixedCellSize(control.getFixedCellSize()));
        suggestionList.getItems().addListener((InvalidationListener) _ -> {
            itemChanged = true;
            updateListHeight();
        });
        suggestionList.getStyleClass().add("autocomplete-list");
        control.suggestionsCellFactoryProperty().addListener((_, _, newVal) -> {
            if (newVal != null) {
    ...
    private void registerEventListener() {
        ...
        control.showingProperty().addListener((_, _, newVal) -> {
            if (newVal) {
                animate();
            }
        });
    ...
    public void animate() {
        ...
        if (showTransition == null || showTransition.getStatus().equals(Status.STOPPED)) {
            ...
            showTransition.setOnFinished((_) -> {
                Group vf = (Group) suggestionList.lookup(".sheet");
                ParallelTransition trans = new ParallelTransition();
                for (var i = 0; i < vf.getChildren().size(); i++) {
                    final var cell = (ListCell<T>) vf.getChildren().get(i);
                    final var index = cell.getIndex();
                    if (index > -1) {
                        cell.setOpacity(0);
                        cell.setTranslateY(-suggestionList.getFixedCellSize() / 8);
                        final var timeline = new Timeline(new KeyFrame(Duration.millis(120),
                                _ -> {
                                    cell.setOpacity(1);
                        ...
                        timeline.setDelay(Duration.millis(index * 20));
                        trans.getChildren().add(timeline);
                    }
                }
    ...
    private void selectItem() {
        ...
        if (item != null) {
            control.getSelectionHandler().handle(new AutoCompletePopup.AutoCompleteEvent<>(AutoCompletePopup.AutoCompleteEvent.SELECTION, item));
        }
    }
